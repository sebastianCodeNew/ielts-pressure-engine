...
def calculate_band_score(raw_value: float, multiplier: float, high: float = 9.0, low: float = 1.0, is_wpm: bool = False) -> float:
...
def get_radar_metrics(signals) -> Dict[str, float]:
    ...
    return {
        "Fluency": calculate_band_score(getattr(signals, 'fluency_wpm', 0.0), WPM_MULTIPLIER, is_wpm=True),
        "Coherence": calculate_band_score(getattr(signals, 'coherence_score', 0.0), COHERENCE_MULTIPLIER),
        "Lexical": calculate_band_score(getattr(signals, 'lexical_diversity', 0.0), LEXICAL_MULTIPLIER),
        "Grammar": calculate_band_score(getattr(signals, 'grammar_complexity', 0.0), GRAMMAR_MULTIPLIER),
        "Pronunciation": calculate_band_score(getattr(signals, 'pronunciation_score', 0.0), PRONUNCIATION_MULTIPLIER)
    }
...
def get_overall_band(metrics: Dict[str, float]) -> float:
    ...
    # IELTS rounds to nearest 0.5 in real life, but for the engine we keep 1 decimal place 
    # to show micro-progress during training.
    return round(avg, 1)
