...
import weakref
# WeakValueDictionary automatically purges locks when the session is no longer active
# (preventing memory leaks over long uptimes)
    ...
    async with lock:
        try:
            ...
            if len(words) > 5 and len(unique_words) / len(words) < 0.3:
                # Highly repetitive text is usually noise (hallucination)
                is_junk = True
            
            # v22.0: Relaxed guard for Part 1 (allow single-word answers like "Yes")
            min_words = 1 if current_part == "PART_1" else 2
            transcript_is_empty = not raw_text.strip() or len(words) < min_words or is_junk
            
            if transcript_is_empty:
                logger.warning(f"Guard triggered (is_junk={is_junk}): transcript empty or noise.")
            ...
            if is_exam_mode:
                ...
                # Use the existing QA row (already flushed above)
                if not new_qa:
                    # Fallback check (safety only)
                    new_qa = db.query(QuestionAttempt).filter(
                        QuestionAttempt.session_id == session_id,
                        QuestionAttempt.part == current_part
                    ).order_by(QuestionAttempt.id.desc()).first()
                ...
                if checkpoint_hits:
                    ...
                    if words_to_translate:
                        translated_hits = await asyncio.gather(*[translate_to_indonesian_async(w) for w in words_to_translate])
                ...
                if required_checkpoint_words and attempt.transcript and attempt.transcript != "[TRANSCRIPTION_FAILED]":
                    ...
                    if missing:
                        ...
                        intervention.refactor_mission = "Checkpoint Words wajib dipakai. Ulangi jawaban, dan gunakan kata ini: " + ", ".join(missing) + "."
                ...
                if intervention.feedback_markdown:
                    ...
                    for error_type in detected_errors:
                        ...
                        if existing:
                            ...
                            db.add(ErrorLog(user_id=exam_session.user_id, error_type=error_type.value, session_id=session_id))
                ...
                if part_count >= 3 and current_part == "PART_1":
                    ...
                    cue_card = random.choice(p2_topics) if p2_topics else {"main_prompt": "Describe a challenge...", "cues": []}
                    ...
                    all_attempts = [a for a in all_attempts_raw if a.transcript and a.transcript != "[TRANSCRIPTION_FAILED]" and a.transcript.strip()]
                    ...
                    if all_attempts:
                        ...
                        exam_session.fluency_score = calculate_band_score(safe_avg([a.wpm for a in all_attempts]), WPM_MULTIPLIER, is_wpm=True)
                        exam_session.coherence_score = calculate_band_score(safe_avg([a.coherence_score for a in all_attempts]), COHERENCE_MULTIPLIER)
                        exam_session.lexical_resource_score = calculate_band_score(safe_avg([a.lexical_diversity for a in all_attempts]), LEXICAL_MULTIPLIER)
                        exam_session.grammatical_range_score = calculate_band_score(safe_avg([a.grammar_complexity for a in all_attempts]), GRAMMAR_MULTIPLIER)
                        exam_session.pronunciation_score = calculate_band_score(safe_avg([a.pronunciation_score for a in all_attempts]), PRONUNCIATION_MULTIPLIER)
                        
                        exam_session.overall_band_score = round((exam_session.fluency_score + exam_session.coherence_score + exam_session.lexical_resource_score + exam_session.grammatical_range_score + exam_session.pronunciation_score) / 5.0, 1)

                        user = db.query(User).filter(User.id == exam_session.user_id).first()
                        if user:
                            all_scores = db.query(ExamSession.overall_band_score).filter(ExamSession.user_id == user.id, ExamSession.overall_band_score.isnot(None)).all()
            ...
            error_intervention = Intervention(
                action_id="MAINTAIN",
                next_task_prompt=current_prompt if 'current_prompt' in locals() else "Continue.",
