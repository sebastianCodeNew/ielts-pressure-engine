...
class ErrorDrill(BaseModel):
    ...
    error_type: str = Field(
        description="The type of error being drilled (e.g., 'Subject-Verb Agreement')"
    )
    sentence_with_error: str = Field(
        description="A sentence containing the common error"
    )
...
async def generate_error_gym_drills(
    error_type: str, error_count: int = 3
) -> ErrorGymSession:
    """
