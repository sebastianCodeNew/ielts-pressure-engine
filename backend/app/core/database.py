...
engine = create_engine(
    settings.DATABASE_URL, connect_args={"check_same_thread": False, "timeout": 120}
)
...
class User(Base):
    ...
    exam_sessions = relationship(
        "ExamSession", back_populates="user", cascade="all, delete-orphan"
    )
...
class QuestionAttempt(Base):
    ...
    session_id = Column(
        String, ForeignKey("exam_sessions.id", ondelete="CASCADE"), index=True
    )
...
def get_columns(conn, table_name):
    try:
        # v25.0: Explicitly fetch all rows immediately to avoid proxy closure issues
...
def init_db():
    ...
    # Using engine.connect().execution_options(autocommit=True) if engine.begin() fails
    try:
        with engine.connect() as conn:
            ...
            for col, col_type in migrations_qa.items():
                if col not in cols_qa:
                    try:
                        conn.execute(
                            text(
                                f"ALTER TABLE question_attempts ADD COLUMN {col} {col_type}"
                            )
                        )
                    except Exception as e:
                        if "duplicate column name" not in str(e).lower():
                            logger.warning(
                                f"Migration error (question_attempts.{col}): {e}"
                            )
            ...
            for col, col_type in migrations_es.items():
                if col not in cols_es:
                    try:
                        conn.execute(
                            text(
                                f"ALTER TABLE exam_sessions ADD COLUMN {col} {col_type}"
                            )
                        )
                    except Exception as e:
                        if "duplicate column name" not in str(e).lower():
                            logger.warning(
                                f"Migration error (exam_sessions.{col}): {e}"
                            )
            ...
            if "target_band" not in cols_u:
                try:
                    conn.execute(
                        text(
                            "ALTER TABLE users ADD COLUMN target_band VARCHAR DEFAULT '9.0'"
                        )
                    )
                except Exception as e:
            ...
            if "weakness" not in cols_u:
                try:
                    conn.execute(
                        text(
                            "ALTER TABLE users ADD COLUMN weakness VARCHAR DEFAULT 'General'"
                        )
                    )
                except Exception as e:
            ...
            for col, col_type in migrations_v.items():
                if col not in cols_v:
                    try:
                        conn.execute(
                            text(
                                f"ALTER TABLE vocabulary_items ADD COLUMN {col} {col_type}"
                            )
                        )
                        ...
                        if col == "ease_factor":
                            conn.execute(
                                text(
                                    "UPDATE vocabulary_items SET ease_factor = 2.5 WHERE ease_factor IS NULL"
                                )
                            )
                        if col == "interval_days":
                            conn.execute(
                                text(
                                    "UPDATE vocabulary_items SET interval_days = 1 WHERE interval_days IS NULL"
                                )
                            )
                    except Exception as e:
                        if "duplicate column name" not in str(e).lower():
                            logger.warning(
                                f"Migration error (vocabulary_items.{col}): {e}"
                            )
            ...
            if "error_type" not in cols_err:
                try:
                    conn.execute(
                        text("ALTER TABLE error_logs ADD COLUMN error_type TEXT")
                    )
                    conn.execute(
                        text(
                            "ALTER TABLE error_logs ADD COLUMN count INTEGER DEFAULT 1"
                        )
                    )
                except Exception as e:
