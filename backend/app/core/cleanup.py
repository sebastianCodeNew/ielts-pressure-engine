...
def cleanup_old_audio(max_age_hours: int = 24, max_size_mb: int = 500):
    ...
    try:
        ...
        if total_size_mb > max_size_mb:
            logger.warning(
                f"Audio storage size ({total_size_mb:.1f}MB) exceeds quota ({max_size_mb}MB). Purging oldest files..."
            )
            ...
            if quota_deleted > 0:
                logger.info(
                    f"--- Quota Cleanup: Removed {quota_deleted} extra files to free space ({total_size / (1024 * 1024):.1f}MB remaining) ---"
                )
