...
audio_limiter = RateLimiter(
    limit=settings.RATE_LIMIT_COUNT, window=settings.RATE_LIMIT_WINDOW_SECONDS
)
...
async def lifespan(app: FastAPI):
    ...
    if (
        not settings.DEEPINFRA_API_KEY
        or settings.DEEPINFRA_API_KEY == "your_api_key_here"
    ):
        logger.warning(
            "CRITICAL: DEEPINFRA_API_KEY is missing or using default. AI features will fail!"
        )
    else:
...
async def global_exception_handler_middleware(request: Request, call_next):
    ...
    if content_length and int(content_length) > settings.MAX_AUDIO_SIZE_BYTES:
        logger.error(
            f"Payload Too Large: {content_length} bytes from {request.client.host}"
        )
        return JSONResponse(
            status_code=413,
            content={
    ...
    try:
        ...
        logger.error(
            f"FATAL UNHANDLED ERROR for {request.url.path}: {e}", exc_info=True
        )
        return JSONResponse(
            status_code=500,
            content={
