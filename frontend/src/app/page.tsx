...
export default function TrainingCockpit() {
  ...
  const [part2Phase, setPart2Phase] = useState<"IDLE" | "PREP" | "SPEAKING">(
    "IDLE",
  );
  ...
  const startExam = async () => {
    try {
      ...
      const initialPrompt =
        ...
        "Welcome to the IELTS Speaking Mock Exam. Let's begin with Part 1. Can you tell me about your hometown?";
        ...
        <div className="z-10 text-center space-y-12">
          <div>
            ...
            <p className="text-zinc-500 font-medium">
              Target Band {targetBand} • Ready to train?
            </p>
          </div>
              ...
              <div className="p-8 border-b border-zinc-800 flex justify-between items-center">
                <div>
                  <h2 className="text-2xl font-bold text-white flex items-center gap-3">
                    <Bookmark className="fill-emerald-500 text-emerald-500" />{" "}
                    Word Vault
                  </h2>
                  <p className="text-zinc-500 text-sm mt-1">
                    Your active collection of high-band vocabulary.
                  </p>
                </div>
                <button
              ...
              <div className="flex-1 overflow-y-auto p-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 custom-scrollbar">
                {loadingVault ? (
                  ...
                  <div className="col-span-full flex flex-col items-center justify-center text-zinc-600 py-20">
                    ...
                    <p className="text-sm">
                      Use the "Idea Generator" during exams to save words.
                    </p>
                  </div>
                ) : (
                  vocabList.map((item, idx) => (
                    <div
                      key={idx}
                      className="bg-zinc-900/50 border border-zinc-800 p-6 rounded-xl hover:border-zinc-600 transition-all group"
                    >
            ...
            <div className="bg-[#18181b] w-full max-w-2xl rounded-3xl border border-zinc-800 p-8 relative">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="p-6 bg-zinc-900 rounded-2xl border border-zinc-800">
                  ...
                  <p
                    className={`text-4xl font-black mt-2 ${finalScore && finalScore >= parseFloat(targetBand) ? "text-emerald-500" : "text-red-500"}`}
                  >
                    {finalScore
                      ? (finalScore - parseFloat(targetBand)).toFixed(1)
                      : "..."}
                  </p>
                </div>
              </div>
    ...
    <div className="min-h-screen bg-[#0d0d12] flex flex-col relative overflow-hidden">
        ...
        <div className="fixed top-20 left-6 z-50 animate-in slide-in-from-left-12 duration-1000">
          <div className="bg-amber-500/10 border border-amber-500/20 p-5 rounded-3xl backdrop-blur-xl max-w-[220px] shadow-2xl shadow-amber-500/10">
            ...
            <p className="text-zinc-400 text-[10px] leading-relaxed font-medium">
              Recurring issue detected. Focus on your{" "}
              <strong>{lastError}</strong> usage this turn.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="w-full h-16 border-b border-zinc-900 flex justify-between items-center px-6 bg-[#0d0d12]/80 backdrop-blur-md z-20">
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-4">
            <span className="text-xs font-black text-red-600 uppercase tracking-widest px-2 py-1 bg-red-600/10 rounded">
              Live Session
            </span>
          ...
          <div className="flex items-center gap-3 bg-zinc-900/50 px-4 py-1.5 rounded-full border border-zinc-800">
            <div className="flex flex-col items-center">
              ...
              <span
                className={`text-[10px] font-bold leading-none mt-1 ${(feedback?.stress_level || 0) > 0.7 ? "text-red-500 animate-pulse" : "text-emerald-500"}`}
              >
      ...
      <div className="flex-1 flex flex-col relative">
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <div
            className={`w-[500px] h-[500px] border border-zinc-800/20 rounded-full flex items-center justify-center transition-all duration-1000 ${isRecording ? "scale-110" : "scale-100"}`}
          >
            <div
              className={`relative w-64 h-64 rounded-full flex items-center justify-center transition-all duration-700 overflow-hidden ${
                isSpeaking
                  ? "bg-red-500/10"
                  : silenceTimer > 4
                    ? "bg-amber-500/10"
            ...
            >
              {/* THE SPHERE CORE */}
              <div
                className={`absolute inset-0 bg-gradient-to-tr transition-all duration-1000 blur-2xl opacity-40 ${
                  isRecording
                    ? "from-red-600 via-transparent to-red-600 animate-pulse"
                    : isSpeaking
              ...
              <div
                className={`relative z-10 w-32 h-32 rounded-full border-2 transition-all duration-500 ${
                  isRecording
                    ? "border-red-500/50 scale-110 shadow-[0_0_50px_rgba(239,68,68,0.2)]"
                    : silenceTimer > 4
                      ? "border-amber-500/50 scale-95 shadow-[0_0_30px_rgba(245,158,11,0.2)]"
        ...
        <div className="absolute top-8 left-8 max-w-xs space-y-2 opacity-50 hover:opacity-100 transition-opacity">
          ...
          <p className="text-zinc-500 text-xs leading-relaxed">
            {examPart === "PART_1" &&
              "Answer directly and naturally. Keep fluency high. Don't overthink."}
            {examPart === "PART_2" &&
              "Speak continuously for 2 minutes. Structure your story: Beginning, Middle, End."}
            {examPart === "PART_3" &&
              "Develop abstract ideas. Use examples. Contrast opinions."}
          </p>
        </div>
        ...
        <div className="relative z-10 flex-1 flex flex-col items-center justify-center p-8 space-y-12">
          ...
          {activeMission.length > 0 && (
            <div className="flex flex-col items-center gap-3 mb-4 animate-in fade-in slide-in-from-top-4 duration-700">
              <div
                className={`text-[10px] font-bold uppercase tracking-widest px-3 py-1 rounded-full border mb-1 flex items-center gap-2 ${isMasteryMode ? "bg-amber-500/20 border-amber-500 text-amber-500 shadow-[0_0_15px_rgba(245,158,11,0.2)]" : "bg-zinc-900/50 border-zinc-800/50 text-zinc-500"}`}
              >
                {isMasteryMode && <Wand2 size={10} className="animate-pulse" />}
                {isMasteryMode
                  ? "Mastery Mode: Use These Now"
          ...
          {feedback?.correction_drill && !isRecording && (
            <div className="max-w-md w-full bg-amber-500/10 border border-amber-500/30 p-4 rounded-2xl animate-in zoom-in-95 duration-500 flex items-start gap-4">
              ...
              <div className="flex-1">
                <h4 className="text-amber-500 text-[10px] font-black uppercase tracking-widest mb-1">
          ...
          <div className="h-24 w-full max-w-md flex flex-col items-center justify-center relative">
            <AudioWaveform
              ...
              audioStream={stream}
              pulsing={isRecording && silenceTimer > 4}
            />
            ...
            {isRecording && (
              <div
                className={`absolute inset-0 -m-4 border-4 rounded-full transition-all duration-700 ${
                  silenceTimer > 4
                    ? "border-amber-500/50 scale-110 shadow-[0_0_30px_rgba(245,158,11,0.3)]"
                    : "border-red-600/30 scale-100"
                }`}
                style={{
                  animation:
                    silenceTimer > 4
          ...
          {examPart === "PART_2" && part2Phase !== "IDLE" ? (
            <div className="w-full max-w-2xl bg-white text-black p-8 rounded-xl shadow-2xl relative overflow-hidden">
              {/* Status Bar */}
              <div
                className={`absolute top-0 left-0 w-full h-2 ${part2Phase === "PREP" ? "bg-blue-500" : "bg-red-500"} transition-colors`}
              />
                ...
                <div className="mt-8 pt-8 border-t border-zinc-200">
                  ...
                  <div className="flex justify-between relative px-4">
                    ...
                    {[
                      ...
                      <div
                        key={i}
                        className={`flex flex-col items-center gap-2 bg-zinc-100 px-2 rounded-lg transition-all duration-700 ${silenceTimer > 4 && timer <= m.time ? "animate-pulse" : ""}`}
                      >
                        <div
                          className={`w-4 h-4 rounded-full border-2 transition-all duration-500 ${timer <= m.time ? "bg-emerald-500 border-emerald-500 scale-125 shadow-[0_0_10px_rgba(16,185,129,0.4)]" : "bg-white border-zinc-300"}`}
                        />
                        <span
                          className={`text-[9px] font-bold uppercase transition-colors ${timer <= m.time ? "text-emerald-600" : "text-zinc-400"}`}
                        >
                          {m.label}
                        </span>
                      </div>
                  ...
                  >
                    ...
                    <ul className="space-y-2">
                      {[
                        ...
                        >
                          <div
                            className={`w-1 h-1 rounded-full transition-all ${silenceTimer > 4 ? "bg-blue-600 scale-150" : "bg-blue-400"}`}
                          />
                          {item}
                        </li>
                      ))}
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {(hintData.vocabulary as string[]).map(
                      (w: string, i: number) => (
                        <button
                          key={i}
                          onClick={(e) => {
              ...
              {!shadowingMode ? (
                <div className="p-6 bg-red-600/5 border border-red-600/20 rounded-2xl relative group overflow-hidden">
                  ...
                  <div className="flex gap-3 mt-6">
                      ...
                      <button
                        onClick={() => {
                          const audio = new Audio(
                            `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000"}${feedback.user_audio_url}`,
                          );
                          audio.play();
                        }}
                    ...
                    >
                      ...
                      />{" "}
                      {isSpeaking ? "Coaching..." : "Band 9 Version"}
                    </button>
                  </div>
                </div>
              ) : (
                <div className="space-y-3 max-h-64 overflow-y-auto pr-2 custom-scrollbar">
                  {getSentences(feedback.ideal_response || "").map((s, i) => (
                    <div
                      key={i}
                      className={`p-4 bg-zinc-900 border rounded-xl flex justify-between items-center group transition-all ${shadowingIndex === i ? "border-red-500 shadow-[0_0_15px_rgba(239,68,68,0.2)] bg-red-500/5" : "border-zinc-800 hover:border-emerald-500/50"}`}
                    >
                      <div className="max-w-[80%] space-y-2">
                        ...
                        {shadowResults[i] && (
                          <div className="flex items-center gap-3">
                            ...
                            >
                              Mastery:{" "}
                              {Math.round(shadowResults[i].mastery_score * 100)}
                              %
                            </span>
                            <div className="h-1 w-24 bg-zinc-800 rounded-full overflow-hidden">
                              <div
                                ...
                                style={{
                                  width: `${shadowResults[i].mastery_score * 100}%`,
                                }}
                              />
                            </div>
                          </div>
                        ...
                        {shadowResults[i] &&
                          shadowResults[i].mastery_score > 0 && (
              ...
              <div className="flex gap-4 mt-8">
                ...
                <button
                  onClick={() => {
                    ...
                    setFeedback({
                      next_task_prompt:
                        "Alright, let's try that again. Focus on the refinement!",
                    });
                    speak(
                      "Alright, let's try that again. Focus on the refinement!",
                    );
            ...
            <div className="text-center">
              ...
              <div className="flex gap-2 justify-center">
                {celebrationKeywords.map((w, i) => (
                  <span
                    key={i}
                    className="text-emerald-400 font-black uppercase text-sm tracking-widest"
                  >
                    {w}
                  </span>
                ))}
...
function PartBadge({
  ...
  return (
    <div
      className={`px-3 py-1 rounded text-[10px] font-black uppercase tracking-widest transition-colors ${isActive ? "bg-zinc-100 text-black" : "bg-zinc-900 text-zinc-600"}`}
    >
      {label}
    </div>
  );
