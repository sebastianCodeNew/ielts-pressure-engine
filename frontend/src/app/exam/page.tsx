...
export default function ExamSimulator() {
  ...
  const startExam = async () => {
    try {
      ...
      setExamPart("PART_1");
      speak("Welcome to the IELTS Speaking Mock Exam. I am your examiner. Let's begin with Part 1. Can you tell me about your hometown?");
    } catch (e) {
      console.error(e);
    }
  };
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-8 animate-in fade-in zoom-in duration-500">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-zinc-500 max-w-md mx-auto">This 15-minute mock exam simulates real IELTS conditions with adaptive AI stress levels.</p>
        </div>
        <button 
          onClick={startExam}
    ...
    <div className="max-w-4xl mx-auto space-y-12 py-8">
      ...
      <div className="bg-[#12121a] border border-zinc-800/50 p-10 rounded-[40px] shadow-2xl relative overflow-hidden group">
        ...
        <div className="space-y-6 relative z-10">
            <span className="px-3 py-1 bg-zinc-800 text-zinc-400 rounded-full text-[10px] font-bold uppercase tracking-widest">Examiner</span>
            <div className="text-2xl font-medium text-zinc-100 leading-relaxed min-h-[100px]">
                {feedback?.next_task_prompt || "Please describe the room you are in right now."}
            </div>
            {isSpeaking && (
                <div className="flex gap-1 items-end h-4">
                    {[1,2,3,4,5].map(i => (
                        <div key={i} className="w-1 bg-red-500 animate-bounce" style={{ animationDelay: `${i * 0.1}s`, height: `${Math.random() * 100}%` }} />
                    ))}
                </div>
            )}
        </div>
      ...
      <div className="flex flex-col items-center gap-8">
        {!isRecording ? (
          ...
          >
            {!isSpeaking && !processing && <div className="absolute inset-0 rounded-full bg-red-600 animate-ping opacity-20 pointer-events-none" />}
            {processing ? <div className="w-10 h-10 border-4 border-white/30 border-t-white rounded-full animate-spin" /> : <Mic2 size={48} className="text-white" />}
          </button>
        ) : (
          <button 
        ...
        <div className="text-center h-12">
            <p className="text-zinc-500 text-sm font-medium uppercase tracking-widest">
                {isRecording ? "Listening... Speak clearly" : (isSpeaking ? "Examiner is speaking..." : "Tap to start responding")}
            </p>
        </div>
      </div>
      ...
      {feedback?.feedback_markdown && (
        <div className="p-6 bg-zinc-900/30 border border-zinc-800 rounded-3xl animate-in slide-in-from-bottom duration-500">
           <h4 className="text-xs font-bold text-zinc-500 uppercase tracking-widest mb-4">Educational Feedback</h4>
